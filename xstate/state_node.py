...
class StateNode:
    ...
    def get_entries(self, entry_action):
        if callable(entry_action):
            return Action(entry_action)
        else:
            return Action(entry_action.get("type"))
    ...
    def __init__(
        ...
        self.entry = (
            [self.get_entries(action) for action in config.get("entry")]
            if config.get("entry")
            else []
        )

        self.exit = (
            [self.get_entries(action) for action in config.get("exit")]
            if config.get("exit")
            else []
        )
